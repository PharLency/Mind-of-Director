...
class ObjathorRetriever:
    def __init__(
        ...
        self.use_text = bool(use_sbert and sbert_model is not None)
        self.sbert_features = None
        ...
        if self.use_text:
            ...
            assert (
                objathor_clip_features_dict["uids"]
                == objathor_sbert_features_dict["uids"]
            )
            assert thor_clip_features_dict["uids"] == thor_sbert_features_dict["uids"]
            objathor_sbert_features = objathor_sbert_features_dict[
                "text_features"
            ].astype(np.float32)
